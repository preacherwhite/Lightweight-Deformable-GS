...
def setup_training(dataset, gaussians, train_gaussians, opt,
                    ...
                    variational_inference, use_torchode, use_contiformer, use_ode_rnn, use_latent_ode_rnn, use_pc_encoder,
    ...
    if use_pc_encoder:
        ...
        transformer_ode = PointPreservingSpUNetODEWrapper(
            ...
            atol=atol,
            use_tanh=use_tanh,
            num_active_odes=num_active_odes  # Pass the new parameter
        ).cuda()
...
def training(dataset, opt, pipe, num_testing_iterations, num_saving_iterations, train_gaussians=False, 
          ...
          log_directory=None, use_torchode=False, use_contiformer=False, use_ode_rnn=False, use_latent_ode_rnn=False, use_pc_encoder=False,
          voxel_size=0.05, spatial_shape=[128, 128, 128],
          rtol=1e-1, atol=1e-1, use_tanh=False, num_active_odes=-1):
    """
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if iteration == warmup_iterations + 1 and not encoder_decoder_frozen:
        ...
        if iteration in testing_iterations:
            ...
            # Save best model based on PSNR (you could change this to a combined metric if desired)
    ...
    if tb_writer:
        tb_writer.close()
    # At the end of the training function, update the print statement:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_iterations', nargs="+", type=int, default= 10)
    ...
    parser.add_argument('--warmup_iterations', type=int, default=0, help="Number of iterations for warmup phase using transformer_only_reconstruction")
    ...
    parser.add_argument('--ode_only_ratio', type=float, default=1, help="Ratio of ODE-only training vs rendering-included training")
    ...
    training(
        ...
        args.log_directory, args.use_torchode, args.use_contiformer, args.use_ode_rnn, args.use_latent_ode_rnn, args.use_pc_encoder,
