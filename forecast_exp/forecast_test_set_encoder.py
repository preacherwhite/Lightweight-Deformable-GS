...
from pointcept.models.sparse_unet.spconv_unet_distributed import SpUNetEncoder, SpUNetDecoder
...
class PointForecastWrapper(nn.Module):
    ...
    def forward(self, input_sequence, past_time_features, future_time_features, past_observed_mask):
        # Input sequence should be of shape (batch_size, input_length, num_points, num_features)
        skips = self.encoder(input_sequence)
        
        skips = self.decoder(skips)
        return self.transformer(skips)
