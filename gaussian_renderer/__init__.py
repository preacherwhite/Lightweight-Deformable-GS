...
def render_ode(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, new_xyz, new_rotation, new_scaling, is_6dof=False,
    ...
    if is_6dof:
        if torch.is_tensor(new_xyz) is False:
            means3D = pc.get_xyz
        else:
            means3D = from_homogenous(
                torch.bmm(new_xyz, to_homogenous(pc.get_xyz).unsqueeze(-1)).squeeze(-1))
    else:
        # ODE gives out the new xyz values directly, so we don't need to add them to the original xyz
        means3D = new_xyz
    #means2D = screenspace_points
    opacity = pc.get_opacity
