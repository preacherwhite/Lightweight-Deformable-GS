...
class DynamicsMLP_ODE_tanh(nn.Module):
    def __init__(self, in_channels, hidden_channels=None, out_channels=None):
        super().__init__()
        hidden_channels = hidden_channels or in_channels
        out_channels = out_channels or in_channels  
...
@MODELS.register_module("SpUNet-v2-torchODE")
class SpUNetBaseWrapTorchODE(nn.Module):
    def __init__(
        self,
        in_channels,
        num_classes,
        ...
        # The bottleneck latent from the encoder has channel dimension channels[self.num_stages - 1].
        # We insert a per-point MLP to model the dynamics at the bottleneck.
        self.dynamics_mlp = DynamicsMLP_ODE_tanh(
            in_channels=channels[self.num_stages - 1],
            hidden_channels=channels[self.num_stages - 1] // 2,
            out_channels=channels[self.num_stages - 1],
        )
    ...
    def forward(self, input_dict, time_emb):
        ...
        # ---- Apply the Bottleneck Dynamics MLP ----
        term = to.ODETerm(self.dynamics_mlp)
        step_method = to.Dopri5(term=term)
        step_size_controller = to.IntegralController(atol=1e-9, rtol=1e-7, term=term)
        solver = to.AutoDiffAdjoint(step_method, step_size_controller)
        #expand time_emb to the same size as x.features
        time_emb = time_emb.expand(x.features.size(0), -1)
        sol = solver.solve(to.InitialValueProblem(y0=x.features, t_eval=time_emb))
        # x_new:  Bottleneck_size * T * Bottleneck_feature_size
        x_new = torch.squeeze(sol.ys)
        # do a transpose to make it T * Bottleneck_size * Bottleneck_feature_size
        ...
        for i in range(len(x_new)):
            x_seq.append(x.replace_feature(x_new[i]))
        # ---------------------------------------------
        x_seq_out = []
        for x in x_seq:
            # Decoder loop with skip connections.
