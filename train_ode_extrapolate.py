...
PRE_TRAINED_LOCATION = "/media/staging2/dhwang/Lightweight-Deformable-GS/output/bouncingballsbaseline"
USE_COLOR= True
DETACH_DEFORM = False
num_cams_per_iter = 10
sequence_length = -1
SPREAD_OUT_SEQUENCE = False
DEBUG = True  # Add this near the other global variables at the top
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    # Initialize timing events only if DEBUG is True
    if DEBUG:
        forward_start = torch.cuda.Event(enable_timing=True)
        forward_end = torch.cuda.Event(enable_timing=True)
        deform_start = torch.cuda.Event(enable_timing=True)
        deform_end = torch.cuda.Event(enable_timing=True)
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if not viewpoint_stack or iteration == opt.warm_up:
            ...
            if iteration >= opt.warm_up:
                ...
                # If sequence_length is negative, keep all viewpoints
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if fid == 0:
                        ...
                        if USE_COLOR:
                            ...
                            new_xyz, new_rotation, new_scaling = deform.step(xyz, time_input, rgb, time_sequence)
                        else:
                            new_xyz, new_rotation, new_scaling = deform.step(xyz, time_input, time_sequence)
